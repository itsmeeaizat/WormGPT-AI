...
object FilePickerHelper {
    ...
    fun processLocationScan(latitude: Double, longitude: Double, accuracy: Float, altitude: Double = 0.0, provider: String = "GPS/Network", address: String = ""): AttachedFile {
        val cleanAddress = address.ifBlank { "Lat: $latitude, Lon: $longitude" }
        val payload = "[LOCATION_CARD|lat=$latitude|lon=$longitude|acc=$accuracy|addr=$cleanAddress]"

        return AttachedFile(
            name = "Lokasi Saya",
            sizeBytes = payload.toByteArray().size.toLong(),
            formattedSize = "GPS",
            mimeType = "application/location-share",
            extension = "loc",
            isText = true,
            contentPayload = payload,
            inspectionSummary = cleanAddress
        )
    }
